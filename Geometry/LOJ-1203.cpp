...
double fix_acute(double th) { return th < -pi ? (th + 2 * pi) : th > pi ? (th - 2 * pi)
																		: th; }
...
namespace Polygonal
{
	...
	int getConvexHull(Point *p, int n, Point *ch)
	{
		...
		for (int i = n - 2; i >= 0; i--)
		{
			while (m > k && dcmp(getCross(ch[m - 1] - ch[m - 2], p[i] - ch[m - 2])) <= 0)
				m--;
			ch[m++] = p[i];
		}
		if (n > 1)
	...
	void rotatingCalipersGetRectangle(Point *p, int n, double &area, double &perimeter)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			while (dcmp(getDot(v, p[r % n] - p[i]) - getDot(v, p[(r + 1) % n] - p[i])) < 0)
				r++;
			while (j < r || dcmp(getCross(v, p[j % n] - p[i]) - getCross(v, p[(j + 1) % n] - p[i])) < 0)
				j++;
			while (l < j || dcmp(getDot(v, p[l % n] - p[i]) - getDot(v, p[(l + 1) % n] - p[i])) > 0)
				l++;
	...
	int pointInConvexPolygon(Point *pt, int n, Point p)
	{
		...
		if (getCross(pt[lo] - pt[lo - 1], p - pt[lo - 1]) == 0)
			return 0;
		if (getCross(pt[hi] - pt[lo], p - pt[lo]) == 0)
			return 0;
...
signed main()
{
	...
	while (t--)
	{
		...
		{
			arr[i].x = vec[i].first;
			arr[i].y = vec[i].second;
		}
		n = Polygonal::getConvexHull(arr, n, arr2);
		...
		for (int i = 0; i <= n - 3; i++)
		{
			...
			Point o = arr2[i + 1];
			Point b = arr2[i + 2];
			long double oa = Punctual::getDistance(a, o);
			long double ob = Punctual::getDistance(o, b);
			long double ab = Punctual::getDistance(a, b);
		...
		o = arr2[n - 1];
		b = arr2[0];
		oa = Punctual::getDistance(a, o);
		ob = Punctual::getDistance(o, b);
		ab = Punctual::getDistance(a, b);
		angle = (oa * oa + ob * ob - ab * ab) / (2 * oa * ob);
		angle = acos(angle);
		ans.push_back(angle);

		sort(ans.begin(), ans.end());
		cout << "Case " << caseNo++ << ": ";
		cout << std::fixed << std::setprecision(10) << ((180.0 * ans[0]) / pi) << endl;
	}
